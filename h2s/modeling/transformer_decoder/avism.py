...
class Avism(nn.Module):
    ...
    def forward(self, frame_query, audio_features):
        ...
        decomp_queries = self.audio_decomp_queries.weight.unsqueeze(1).repeat(1, T, 1)
        decomp_pos = self.audio_decomp_pos.weight.unsqueeze(1).repeat(1, T, 1)         
        ...
        decomposed_audio = decomposed_audio.permute(1, 0, 2).contiguous()[:, :, None,:].repeat(1, 1, L*B, 1)
