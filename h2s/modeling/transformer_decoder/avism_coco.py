...
from .mambablocks import BiSTSSMBlock
...
class Avism_COCO(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.audio_decomp_queries = nn.Embedding(
            num_queries, hidden_dim
        )  # 100维的可学习查询
        self.audio_decomp_pos = nn.Embedding(num_queries, hidden_dim)  # 对应的位置编码

        self.audio_decomp_attention = CrossAttentionLayer(
        ...
        if enc_layers > 0:
            self.encode_av_fusion = nn.ModuleList()
            for _ in range(self.enc_layers):
                self.encode_av_fusion.append(
                    BiSTSSMBlock(
                        hidden_dim=hidden_dim,
                        normalize_before=pre_norm,
                        forward_type="v2_avis",
                    )
                )
    ...
    def forward(self, frame_query, audio_features):
        ...
        decomp_queries = self.audio_decomp_queries.weight.unsqueeze(1).repeat(
            1, T, 1
        )  # [100, B, hidden_dim]
        decomp_pos = self.audio_decomp_pos.weight.unsqueeze(1).repeat(
            1, T, 1
        )  # [100, B, hidden_dim]

        # audio_feat = self.av_proj(audio_features)#.sum(dim=1)  # T, 8, C 5,8,256

        # T 8 C -> T, fq, LB, C

        audio_feat = self.av_proj(audio_features).permute(1, 0, 2).contiguous()
        # audio_feat = audio_feat[None, :, :]

        decomposed_audio = self.audio_decomp_attention(  #! 100, LB, C
            tgt=decomp_queries,  # query: 可学习的100维查询
            memory=audio_feat,  # key & value: 音频特征
            query_pos=decomp_pos,  # query的位置编码
        )  # 100, T, C

        decomposed_audio = (
            decomposed_audio.permute(1, 0, 2)
            .contiguous()[:, :, None, :]
            .repeat(1, 1, L * B, 1)
        )
        # print('decomposed_audio', decomposed_audio.shape)
        # print('quer', self.num_queries)
        # audio_feat = self.av_proj_2(audio_feat.permute(0, 2, 1)).squeeze(2) # T, C 5,256
        # audio_feat = audio_feat[:, None, None, :].repeat(1, fQ, L * B, 1) #! T, fQ, LB, C  5,100,3,256

        if self.window_size > 0:
            pad = int(ceil(T / self.window_size)) * self.window_size - T
            _T = pad + T
            frame_query = F.pad(
                frame_query, (0, 0, 0, 0, 0, 0, 0, pad)
            )  # _T, fQ, LB, C
        ...
        #! frame_query T, fQ, LB, C
        #! decomposed_audio T, fQ, LB, C

        # audio
        # print("enc_layers:", self.enc_layers)
        for en_av in self.encode_av_fusion:
            frame_query = en_av(frame_query, decomposed_audio)
        ...
        dec_pos = (
            self.fq_pos.weight[None, :, None, :].repeat(T, 1, L * B, 1).flatten(0, 1)
        )  # TfQ, LB, C
        ...
        return [
            {
                ...
                "pred_mask_embed": b,
                "pred_cq_embed": c,
                "pred_fq_embed": outputs_fq_embed,
            }
            for a, b, c in zip(
                outputs_cls[:-1], outputs_mask_embed[:-1], outputs_cq_embed[:-1]
            )
        ]
    ...
    def encode_frame_query(self, frame_query, attn_mask):
        ...
        # Using window-based attention if self.window_size > 0.
            ...
            window_mask = (
                attn_mask.view(LB * Nw, W)[..., None].repeat(1, 1, fQ).flatten(1)
            )

            _attn_mask = torch.roll(attn_mask, half_W, 1)
            _attn_mask = _attn_mask.view(LB, Nw, W)[..., None].repeat(
                1, 1, 1, W
            )  # LB, Nw, W, W
            ...
            _attn_mask[:, 0, half_W:, :half_W] = True
            _attn_mask = (
                _attn_mask.view(LB * Nw, 1, W, 1, W, 1)
                .repeat(1, self.num_heads, 1, fQ, 1, fQ)
                .view(LB * Nw * self.num_heads, W * fQ, W * fQ)
            )
            ...
            for layer_idx in range(self.enc_layers):
                if self.training or layer_idx % 2 == 0:
                    ...
                    frame_query = self._shift_window_attn(
                        frame_query, shift_window_mask, layer_idx
                    )
            return frame_query
    ...
    def _window_attn(self, frame_query, attn_mask, layer_idx):
        ...
        frame_query = self.enc_self_attn[layer_idx](
            frame_query, tgt_key_padding_mask=attn_mask
        )
        frame_query = self.enc_ffn[layer_idx](frame_query)
        frame_query = (
            frame_query.reshape(W, fQ, LB, Nw, C)
            .permute(3, 0, 1, 2, 4)
            .reshape(T, fQ, LB, C)
        )
