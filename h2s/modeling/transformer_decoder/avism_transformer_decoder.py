...
from .semantic_aware_sparseattn import SemanticAwareSparseAttention
...
class AVISMMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.audio_decomp_queries = nn.Embedding(num_queries, hidden_dim)  # 100维的可学习查询
        ...
        self.av_sf.append(SemanticAwareSparseAttention(
    ...
    def forward(self, x, mask_features, clip_mask_features, audio_features, mask = None, return_attention_viz=False):
        ...
        decomp_queries = self.audio_decomp_queries.weight.unsqueeze(1).repeat(1, bs, 1)  # [100, B, hidden_dim]
        decomp_pos = self.audio_decomp_pos.weight.unsqueeze(1).repeat(1, bs, 1)          # [100, B, hidden_dim]

        frame_queries = []
        predictions_class = []
        predictions_mask = []

        audio_feat = self.av_pre_proj(audio_features).permute(1, 0, 2)

        decomposed_audio = self.audio_decomp_attention(
            tgt=decomp_queries,           
            memory=audio_feat,            
            query_pos=decomp_pos          
        )

        av_feats = []
        attention_info_list = [] if return_attention_viz else None
        for l in range(len(src)):
            if return_attention_viz:
                # 传入真实的空间尺寸 spatial_shapes
                av_feat, attn_info = self.av_sf[l](decomposed_audio, src[l], src[l], query_pos=decomp_pos, return_attention_info=True, spatial_shapes=size_list[l])
                attention_info_list.append(attn_info)
            else:
                av_feat = self.av_sf[l](decomposed_audio, src[l], src[l], query_pos=decomp_pos)
        ...
        for i in range(self.num_layers):
            ...
            output = self.transformer_cross_attention_layers[i](
                ...
                pos=pos[level_index], query_pos=decomp_pos
            )
        ...
        if return_attention_viz:
            out['attention_viz_info'] = attention_info_list
