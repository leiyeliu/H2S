...
class AVISM(nn.Module):
    ...
    def inference(self, batched_inputs):
        ...
        with torch.no_grad():
            for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
                images = batched_inputs["image"][
                    i * self.test_run_chunk_size : (i + 1) * self.test_run_chunk_size
                ]
                images = [
                ...
                audio_features_chunk = audio_features[
                    i * self.test_run_chunk_size : (i + 1) * self.test_run_chunk_size
                ]

                features = self.backbone(images.tensor)
                outputs, _frame_queries, _mask_features = self.sem_seg_head(
        ...
        for i in range(
            ...
            m_f = mask_features[
                i * self.test_interpolate_chunk_size : (i + 1)
