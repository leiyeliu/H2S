...
from .metrics import Count, combine_av_loc_sequences, compute_av_loc
            ...
            for tracker in tracker_list:
                ...
                try:
                    ...
                    for curr_seq in tqdm.tqdm(seq_list_sorted):
                        ...
                        res_av_loc[curr_seq] = eval_av_loc_sequence(
                            curr_seq, dataset, tracker
                        )
                    ...
                    res_output["AP_all"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["TrackMAP"]["AP_all"]
                        ),
                        2,
                    )
                    res_output["AP_s"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["TrackMAP"][
                                "AP_area_s"
                            ]
                        ),
                        2,
                    )
                    res_output["AP_m"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["TrackMAP"][
                                "AP_area_m"
                    ...
                    res_output["AR_all"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["TrackMAP"]["AR_all"]
                        ),
                        2,
                    )

                    res_output["HOTA"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["HOTA"]
                        ),
                        2,
                    )
                    res_output["DetA"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["DetA"]
                        ),
                        2,
                    )
                    res_output["DetRe"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["DetRe"]
                        ),
                        2,
                    )
                    res_output["DetPr"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["DetPr"]
                        ),
                        2,
                    )
                    res_output["AssA"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["AssA"]
                        ),
                        2,
                    )
                    res_output["AssRe"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["AssRe"]
                        ),
                        2,
                    )
                    res_output["AssPr"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["AssPr"]
                        ),
                        2,
                    )
                    res_output["LocA"] = round(
                        100
                        * np.mean(
                            res["COMBINED_SEQ"]["cls_comb_cls_av"]["HOTA"]["LocA"]
                        ),
                        2,
                    )
                    ...
                    # Added: Calculate per-sequence final metrics
                    per_seq_computed = {}
                    for curr_seq in seq_list:
                        per_seq_computed[curr_seq] = {}
                        for metric, metric_name in zip(metrics_list, metric_names):
                            cls_res = {}
                            for cls in class_list:
                                cls_res[cls] = metric.combine_sequences(
                                    {curr_seq: res[curr_seq][cls][metric_name]}
                                )
                            per_seq_computed[curr_seq][metric_name] = (
                                metric.combine_classes_class_averaged(cls_res)
                            )
                    res_output["per_seq_computed"] = per_seq_computed

                    # Added by GitHub Copilot to export per-sequence results
                    res_output["per_seq_res"] = res
                    res_output["per_seq_res_av_loc"] = res_av_loc
