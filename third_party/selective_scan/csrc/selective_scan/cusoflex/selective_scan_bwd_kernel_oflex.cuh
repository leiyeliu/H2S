...
struct Selective_Scan_bwd_kernel_traits {
    ...
    static constexpr int kNBytesOutput = sizeof(output_t);
    static constexpr int kNEltsOutput = kNBytesOutput == 4 ? 4 : std::min(8, kNItems);
    using vec_output_t = typename BytesToType<kNBytesOutput * kNEltsOutput>::Type;
    using scan_t = float2;
    ...
    using BlockLoadOutputVecT = cub::BlockLoad<vec_output_t, kNThreads, kNLoadsOutput, cub::BLOCK_LOAD_WARP_TRANSPOSE>;
