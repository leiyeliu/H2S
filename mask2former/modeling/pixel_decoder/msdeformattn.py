...
from .bicrossattn import BidirectionalCrossAttention
...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def forward_features(self, features, audio_features=None):
        srcs1 = []
        pos = []
        # Reverse feature maps into top-down order (from low to high resolution)
        for idx, f in enumerate(self.transformer_in_features[::-1]):
            x = features[f].float()  # deformable detr does not support half precision
            srcs1.append(self.input_proj[idx](x))
            pos.append(self.pe_layer(x))

        srcs = []
        # print("audio_features.shape", audio_features.shape)
        # audio_features = audio_features.unsqueeze(1)
        for i, src in enumerate(srcs1):
            _, C, H, W = src.shape
            src = src.flatten(2).transpose(1, 2)
            po = pos[i].flatten(2).transpose(1, 2)
            # print('src shape:', src.shape)
            # print('pos shape:', po.shape)
            # print('audio_features shape:', audio_features.shape)
            src, tmp_a = self.bi_attn[i](src, audio_features, pos=po)
            srcs.append(src.transpose(1, 2).view(-1, C, H, W))
